...
public class chatwithbuyersCA extends RecyclerView.Adapter<chatwithbuyersCA.facilityCAViewHolder>{
    ...
    private chatwithbuyersCA.OnItemClickListener mListener;
    ...
    public void setOnItemClickListener(chatwithbuyersCA.OnItemClickListener listener){
        mListener = listener;
    }

    public chatwithbuyersCA(Context context, ArrayList<messagechatDMC> ListfacilityDCMList){
        mContext = context;
...
//TODO ensure that when registering LANDLOARDS you generate their unique ids that will not bring conflict as it is now
    ...
    public void onBindViewHolder(final facilityCAViewHolder holder, final int position) {
        ...
        try {
            String userLoggedinID = sharedPreference.getUSERDETAILS().getString("phoneNo");
